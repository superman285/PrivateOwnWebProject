...
router.get('/checkUserName', async ctx => {
    ...
    console.log('regist取到的user',user);
...
router.get('/checkUserName_login',async ctx=>{
    ...
    let sql1 = `select * from users where username=?`;
    let [[user]] = await db.query(sql1,[username]);
    console.log("解构取到的user:",user);

    if (user) {
        ctx.body = {
            code: 0,
            message: '该用户名已注册,可以登录'
        };
        return;
    }
...
router.post('/login', async ctx => {
    ...
    console.log("ctx.request.body是：",ctx.request.body);
    ...
    if (!username || !password) {
        ...
        //console.log("ctx.body即ctx.response.body:\n",ctx.body);
        return;
    }
